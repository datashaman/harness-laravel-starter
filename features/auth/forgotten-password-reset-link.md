...
| AC-2  | Returns 204 regardless of whether the email is registered  | security | Must     |
| AC-3  | Sends a reset link by email when the address matches a user | feature  | Must     |
| AC-4  | Reset links are single-use                                 | security | Must     |
| AC-5  | Reset links expire 60 minutes after issue                  | security | Must     |
| AC-6  | Rate-limits to one request per email address per minute    | security | Must     |
| AC-7  | Rate-limits to 10 distinct addresses per IP per 5 minutes  | security | Must     |
